...
def read_matrix(numberOfBlocks):
...
def is_goal_state(matrix:List[List[int]], numberOfBlocks:int, emptyBlockPos):
    ...
    isGoalBlancPosReached = matrix[empty_row][empty_col] == 0

    result = 0
    if isGoalBlancPosReached:
...
def depth_limited_search(startNode: Node, threshold: int, sizeOfRowCol: int, emptyBlockPos:int ):
    stack = [startNode]  # Using list as a stack
    ...
    while stack:
        ...
        if f_cost > threshold:
            ...
            continue
        if is_goal_state(node.state, sizeOfRowCol ** 2 - 1, emptyBlockPos):
            return node

        blank_x, blank_y = next((x, y) for x in range(sizeOfRowCol) for y in range(sizeOfRowCol) if node.state[x][y] == 0)

        for move_name, (dx, dy) in MOVES.items():

            new_x, new_y = blank_x + dx, blank_y + dy

            if 0 <= new_x < sizeOfRowCol and 0 <= new_y < sizeOfRowCol:
                new_state = [row[:] for row in node.state] # creating a shallow copy in row
                new_state[blank_x][blank_y], new_state[new_x][new_y] = new_state[new_x][new_y], new_state[blank_x][blank_y]

                if node.parent and node.parent.state == new_state:
                    continue

                new_node = Node(new_state, node.gCost + 1, move_name, node)
                stack.append(new_node)

    return None if min_f_cost == float('inf') else min_f_cost

def ida_star(matrix: List[List[int]], sizeOfRowCol: int, emptyBlockPos:int):
    ...
    while True:
        result = depth_limited_search(startNode, threshold, sizeOfRowCol, emptyBlockPos)
...
def main() -> None:
    ...
    if is_goal_state(matrix, N, emptyBlockPos):
        print("The matrix is already in the goal state!")
    else:
        start_time = time.time()

        solution = ida_star(matrix, sizeOfRowCol,emptyBlockPos)
...
def test_isGoalState1():
    ...
    print(False == is_goal_state(matrix,N,emptyBlockPos))

def test_isGoalState2():
    N = 8
    emptyBlockPos = 8
    matrix = [[1,2,3],[4,5,6],[7,8,0]]
    print(True == is_goal_state(matrix,N,emptyBlockPos))
...
def test_ida_star1():
    ...
    sizeOfRowCol = int(math.sqrt(N + 1))
    matrix = [[1,2,3],[4,5,6],[0,7,8]]
    temp = ida_star(matrix,sizeOfRowCol, emptyBlockPos)
    expected = ['left', 'left']
    print(temp == expected)
